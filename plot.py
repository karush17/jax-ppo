...
rc('font',**{'family':'serif','sans-serif':['Helvetica']}, weight='bold', size=20)
rc('text', usetex=True)


def moving_average(a, n=20,diff=False):
    if diff==True:
...
for env_name in env_list:
    ...
    plt.plot(np.arange(0,len(il_reward),1),il_reward, color='darkorange', linewidth=4, label='PPO')
    plt.fill_between(np.arange(0,len(il_reward),1),il_rew_min,il_rew_max,facecolor='darkorange', alpha=alpha, linewidth=0, antialiased=True)
    plt.ylabel('Returns')
